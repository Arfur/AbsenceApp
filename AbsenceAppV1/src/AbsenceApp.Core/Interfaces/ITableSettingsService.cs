...
public interface ITableSettingsService
{
    ...
    /// Returns schema-driven defaults (from EF Core metadata) when no settings have been persisted yet.
    /// </summary>
    ...
    /// reverting the page to schema-driven defaults on next load.
    /// </summary>
    Task ResetToDefaultsAsync(string pageName);

    /// <summary>
    /// Compares EF Core metadata against persisted table_page_settings rows for every
    /// mapped page and returns a structured drift report.
    /// </summary>
    Task<TableSettingsDiagnosticReport> GetDiagnosticsAsync();
}
