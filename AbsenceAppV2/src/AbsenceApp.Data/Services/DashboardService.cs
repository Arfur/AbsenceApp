...
public class DashboardService : IDashboardService
{
    ...
    public async Task<DashboardOverviewDto> GetOverviewAsync()
    {
        ...
        var absencesToday     = await _context.StudentAbsences.CountAsync(a => a.Date == today);
        var unauthorisedToday = await _context.StudentAbsences.CountAsync(a => a.Date == today && !a.IsAuthorised);
    ...
    {
        var result = await _context.StudentAbsences
            .GroupBy(a => a.StudentId)
            .Select(g => new
            {
                StudentId    = g.Key,
                AbsenceCount = g.Count(),
                LastAbsence  = g.Max(a => a.Date)
            })
