...
public class AuditLogRepositoryTests
{
    ...
    public async Task Query_FilterByUserId_ShouldReturnMatchingLogs()
    {
        ...
        var repo = new AuditLogRepository(ctx);
        await repo.AddAsync(new AuditLog { AuditId = 1, UserId = userId1, Action = "A1", Timestamp = DateTime.UtcNow });
        await repo.AddAsync(new AuditLog { AuditId = 2, UserId = userId1, Action = "A2", Timestamp = DateTime.UtcNow });
        await repo.AddAsync(new AuditLog { AuditId = 3, UserId = (int)u2.Id, Action = "B1", Timestamp = DateTime.UtcNow });
