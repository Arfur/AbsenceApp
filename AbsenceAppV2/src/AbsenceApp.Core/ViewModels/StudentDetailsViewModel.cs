...
public class StudentDetailsViewModel
{
    ...
    /// <summary>Loads the student for the given string ID (parsed to long internally).</summary>
    public async Task LoadAsync(string id)
    {
        if (!long.TryParse(id, out var studentId))
        {
            ErrorMessage = $"Invalid student ID: '{id}'.";
            return;
        }
        IsLoading = true;
        ErrorMessage = null;
